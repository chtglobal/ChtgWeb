...
function wpmu_delete_blog( $blog_id, $drop = false ) {
	...
	if ( $drop ) {
		...
		$dir = apply_filters( 'wpmu_delete_blog_upload_dir', WP_CONTENT_DIR . " s.dir/{$blog_id}/files/", $blog_id );
...
function ms_deprecated_blogs_file() {
	if ( ! is_super_admin() )
		return;
	if ( ! file_exists( WP_CONTENT_DIR . ' s.php' ) )
		return;
	echo '<div class="update-nag">' . sprintf( __( 'The <code>%1$s</code> file is deprecated. Please remove it and update your server rewrite rules to use <code>%2$s</code> instead.' ), 'wp-content s.php', 'wp-includes/ms-files.php' ) . '</div>';
}
